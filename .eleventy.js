...
module.exports = function (eleventyConfig) {
  ...
  const latestDateComp = (a, b) => {
    return a.date > b.date ? -1 : 1;
  };
  const earliestDateComp = (a, b) => {
    return a.date > b.date ? 1 : -1;
  };
  const titleComp = (a, b) => {
    return a.data.title < b.data.title ? -1 : 1;
  };
  const seriesOrderComp = (a, b) => {
  ...
  async function getSrcsetSizes(path, w, fullWidth, n) {
    ...
    const bigSize =
      n == 1 ? rowWidth : n == 2 ? Math.round(rowWidth / 2) : Math.round(rowWidth / 3);
  ...
  async function img2(imgs, options = {}) {
    if (typeof options === "boolean") {
      console.warn(
        "The first argument passed to img2 was a boolean, which is the wrong call signature."
      );
    }
   ...
   * @returns metadata
   */
  async function imgCommon(path, widths) {
  ...
  eleventyConfig.addShortcode("rawImg", async function (path, widths, sizes, classes) {
    const metadata = await imgCommon(path, widths);
    ...
    if (thumbhash64 != null) {
      options["data-thumbhash-b64"] = thumbhash64;
    }
    return Image.generateHTML(metadata, options);
  });

  /**
   * NOTE: This isn't really thumb-specific, just uses 100vw. Could rename to
   * make clear for any size, or even pass in widths.
   */
  eleventyConfig.addShortcode(
    "thumb",
    async function (path, widths, classes = "", style = "") {
  ...
  eleventyConfig.addShortcode("sketch", async function (path, classes = "") {
    return await commonImg(
      path,
      "mh-100vh w-auto h-auto pa1 pa3-m pa4-l border-box " + classes
    );
  });

  eleventyConfig.addShortcode(
    "coverImg",
    async function (path, classes = "", style = "") {
  ...
  eleventyConfig.addShortcode(
    ...
    async function (
      ...
      const figClasses =
        mt && mb ? "fig" : mt && !mb ? "figtop" : mb && !mt ? "figbot" : "";
      ...
      imgExClasses =
        !embedded && !plainBig && imgExClasses == "" ? "pv4 pv5-ns" : imgExClasses;
      ...
      basePieces.push(
        `<div style="${containerStyleSize} ${containerXStyle}" class="${containerXClasses}">`
      );
      for (let i = 0; i < paths.length; i++) {
        ...
        let [srcSet, sizes] = await getSrcsetSizes(
          paths[i],
          w,
          embedded || plainBig,
          embedded ? 3 : 1
        );
        const imgXClasses = isX
          ? `fader z-${i} o-${i == paths.length - 1 ? 1 : 0}`
          : "";
        const imgXStyleAttr = isX
          ? `style="grid-area: 1 / 1 / 2 / 2; transition: opacity 0.75s;"`
          : `style=""`;
        basePieces.push(`<img
            src="${paths[i]}"
                class="novmargin h-auto ${imageClasses} ${imgXClasses} ${imgExClasses} ${
          i == 0 ? firstImgClass : ""
        }"
                ${imgXStyleAttr}
                loading="lazy" decoding="async"
  ...
  eleventyConfig.addShortcode("cityPic", async function (path) {
    ...
    const content = await oneBigImage2(path, getMarginClasses(1, 2), {
      fullWidth: true,
    });
    return "<md-raw>" + content + "</md-raw>";
  });
...
${third}`;
  ...
  markdownLibrary.renderer.rules.footnote_caption = (
    tokens,
    idx /*, options, env, slf*/
  ) => {
    var n = Number(tokens[idx].meta.id + 1)
      .toString()
      .padStart(2, "0");
