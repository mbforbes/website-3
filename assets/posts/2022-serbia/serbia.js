...
// execution
async function main() {
    await loadBoundaries(['Serbia']);
    // const photos = await loadLocations("2022-scotland-photo-locations-shuffled.json");
    const places = [
        [44.8167, 20.4667, "Belgrade", "right", "in"],
        [45.2644, 19.8317, "Novi Sad", "right", "in"],
    ];

    // mapContext: zoomed out
    // ---
    let mapContext = await renderMap("mapContext");
    let contextBoundary = await renderOverlay(mapContext, 'Serbia', "mapOutlineContext", "#FF4136");
    ...
    let tripOverlay = await renderOverlay(mapTrip, 'Serbia', "mapOutlineTravel", "#FF4136");
    let tripBounds = tripOverlay.getBounds().pad(0.05);  // probably good universally
