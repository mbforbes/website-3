...
function getCanvasSize() {
    ...
    return Math.min(800, d);
}
function makeSketch(canvas_w = 500, canvas_h = 500) {
    return (p5js) => {
        // module aliases
        let Engine = Matter.Engine, Body = Matter.Body, World = Matter.World, Bodies = Matter.Bodies, Composite = Matter.Composite, Mouse = Matter.Mouse, MouseConstraint = Matter.MouseConstraint;
        // create an engine
        ...
        // var ballA = Bodies.circle(350, 150, 40);
        ...
        p5js.setup = () => {
            ...
            // left wall
            let leftWall = Bodies.rectangle(-10, canvas_w / 2, 19, canvas_h, { isStatic: true });
            let rightWall = Bodies.rectangle(canvas_w + 10, canvas_w / 2, 19, canvas_h, { isStatic: true });
            ground = Bodies.rectangle(400, 510, 610, 60, { density: 0.01 });
            ground.metadata = 'ground';
            World.add(engine.world, [leftWall, ground, rightWall]);
            // add mouse control
        ...
        p5js.draw = () => {
            mouse.position.x = p5js.mouseX;
            mouse.position.y = p5js.mouseY;
            // maybe add more
            time += 16.667;
            if (time > interval && !dumped) {
                limesDeployed += 1;
                time -= interval;
                World.add(engine.world, Bodies.circle(350, 150, 40));
            }
            // tick physics
            Engine.update(engine, 1000 / 60);
            // manually move body
            // Body.setPosition(ground, Matter.Vector.create(ground.position.x + 2, ground.position.y));
            // turn off its gravity
            Body.applyForce(ground, ground.position, {
                x: -engine.world.gravity.x * engine.world.gravity.scale * ground.mass,
                y: -engine.world.gravity.y * engine.world.gravity.scale * ground.mass,
            });
            // try to stop rotation
            Body.setAngularVelocity(ground, 0);
            // render
            p5js.clear();
            ...
            for (let body of bodies) {
                ...
                if (body.circleRadius != null) {
                    p5js.fill(p5js.color(192, 255, 0));
                    p5js.circle(body.position.x, body.position.y, body.circleRadius * 2);
                    if (body.position.y > canvasSize) {
                        failed = true;
                    }
                }
                else {
                    let vs = body.vertices;
                    for (let i = 0; i < vs.length; i++) {
                        let j = (i + 1) % vs.length;
                        p5js.line(vs[i].x, vs[i].y, vs[j].x, vs[j].y);
                    }
                    // @ts-ignore
                    if (body.metadata && body.metadata == 'ground') {
                        p5js.textFont('monospace');
                        p5js.fill('white');
                        p5js.textAlign('center', 'center');
                        p5js.textSize(20);
                        p5js.text('click and drag me to catch', body.position.x, body.position.y);
                    }
                }
            }
            if (failed) {
                p5js.fill('white');
                p5js.textFont('impact');
                p5js.textAlign('center', 'center');
                p5js.textSize(75);
                p5js.text('why cant you,', canvasSize / 2, 50);
                p5js.text('hold all these limes?', canvasSize / 2, canvasSize - 100);
                p5js.fill('white');
                p5js.textFont('monospace');
                p5js.textAlign('center', 'center');
                p5js.textSize(20);
                p5js.text('u held ' + limesDeployed + ' limes', canvasSize / 2, canvasSize / 2);
            }
            if (failed && !dumped) {
                for (let i = 0; i < canvasSize; i += 35) {
                    for (let j = -300; j <= -150; j += 35) {
                        World.add(engine.world, Bodies.circle(i, j, 40));
                    }
                }
                dumped = true;
            }
